...
function updateTableWindow() {
  ...
  updateTitleBlock(tableView, {
    eyebrow: [manufacturer, year ? String(year) : "", tableType]
      .filter(Boolean)
      .join(" / "),
    title,
    authors,
...
function updateBGWindow() {
  ...
  if (existingMedia) {
    ...
    if (hasUsableMedia(bgVideoUrl) && currentSrc === bgVideoUrl) return;
    if (
  ...
  if (hasUsableMedia(bgVideoUrl)) {
    const video = document.createElement("video");
    video.src = bgVideoUrl;
    video.poster = bgImageUrl;
    video.autoplay = true;
    video.loop = true;
    video.muted = true;
  ...
  } else if (hasUsableMedia(bgImageUrl)) {
    const img = document.createElement("img");
    img.src = bgImageUrl;
    img.alt = title;
    img.style.cssText = "width: 100%; height: 100%; object-fit: cover;";
...
function updateDMDWindow() {
  ...
  // Prefer video if available, fallback to image
  if (hasUsableMedia(dmdVideoUrl)) {
    const video = document.createElement("video");
    video.src = dmdVideoUrl;
    video.poster = dmdImageUrl;
    video.autoplay = true;
    video.loop = true;
    video.muted = true;
    video.playsInline = true;
    ...
    container.appendChild(video);
  } else if (hasUsableMedia(dmdImageUrl)) {
    const img = document.createElement("img");
    img.src = dmdImageUrl;
    img.alt = title;
    img.style.cssText = "width: 100%; height: 100%; object-fit: cover;";
...
function getTableSubtitle() {
  ...
  const vpx = table.meta.VPXFile || {};
  const manufacturer =
    info.Manufacturer || vpx.manufacturer || "Unknown manufacturer";
  const year = info.Year || vpx.year || "";
