...
function updateTableWindow() {
    ...
    if (tableView.manufacturerSeparator) {
        // Hide separator if either manufacturer OR year is empty
        tableView.manufacturerSeparator.style.display = (manufacturer && year) ? '' : 'none';
    }
    if (tableView.versionSeparator) {
        // Hide separator if either version OR authors is empty
        tableView.versionSeparator.style.display = (versionDisplay && authors) ? '' : 'none';
    }
...
function truncateAuthors(authors) {
    ...
    if (typeof authors === 'string') {
        authorArray = authors.split(',').map(a => a.trim()).filter(a => a);
    }
